...
def test_write_diary_on_txt():
    ...
    m = mock_open()
    # Patch sia 'open' nel contesto giusto che 'io.BufferedWriter'
    with patch('diary.diary_txt.open', m), patch('io.BufferedWriter', MagicMock()) as mock_buffer_writer:
...
def test_write_diary_on_txt_ioerror():
    ...
    with patch('diary.diary_txt.open', m):
        write_diary_on_txt(mock_path, mock_diary_content)
